...
function Register() {
  ...
  useEffect(() => {
    // useEffect hook to check for user and redirect to home page
    if (isError) {
      toast.error(message);
    }
    if (isSuccess || user) {
  ...
  const onChange = (e) => {
    // onChange function to update state as user types into form
  ...
  const onSubmit = (e) => {
    // onSubmit function to send form data to backend
    e.preventDefault();
      ...
      <section className=" w-2/3 m-auto">
        <form onSubmit={onSubmit}>
          ...
          <div className=" mb-3 flex items-center justify-center">
            <button
              type="submit"
              className=" px-3 py-5 border-2 border-black rounded-xl bg-blue-700 text-white font-bold cursor-pointer w-1/2"
            >
              Submit
            </button>
          </div>
