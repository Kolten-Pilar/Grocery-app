...
function Register() {
  ...
  const {user, isLoading, isError, isSuccess, message} = useSelector((state) => state.auth) // Destructure auth state

  useEffect(() => { // useEffect hook to check for user and redirect to home page
    if(isError) {
      toast.error(message)
    }
    if(isSuccess || user) {
